...
public class BoatPredictionEngine extends PredictionEngine {
        ...
        for (int k1 = i; k1 < j; ++k1) {
            for (int l1 = k; l1 < l; ++l1) {
                for (int i2 = i1; i2 < j1; ++i2) {
                    double level = player.world.getWaterFluidLevelAt(k1, l1, i2);
                    if (d0 < l1 + level) {
                        if (!player.world.isWaterSourceBlock(k1, l1, i2)) {
    ...
    private static boolean checkInWater(GrimPlayer grimPlayer) {
        ...
        for (int k1 = i; k1 < j; ++k1) {
            for (int l1 = k; l1 < l; ++l1) {
                for (int i2 = i1; i2 < j1; ++i2) {
                    double level = grimPlayer.world.getWaterFluidLevelAt(k1, l1, i2);
                    if (level > 0) {
            ...
            for (int i2 = i1; i2 < j1; ++i2) {
                ...
                if (j2 != 2) {
                    for (int k2 = k; k2 < l; ++k2) {
                        if (j2 <= 0 || k2 != k && k2 != l - 1) {
                            WrappedBlockState blockData = player.world.getBlock(l1, k2, i2);
    ...
    public float getWaterLevelAbove(GrimPlayer player) {
        ...
        for (int k1 = k; k1 < l; ++k1) {
            ...
            for (int l1 = i; l1 < j; ++l1) {
                for (int i2 = i1; i2 < j1; ++i2) {
                    double level = player.world.getWaterFluidLevelAt(l1, k1, i2);
