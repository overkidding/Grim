...
public class ConsumesBlockPlace {
    public static boolean consumesPlace(GrimPlayer player, WrappedBlockState state, BlockPlace place) {
        ...
        if (BlockTags.CANDLE_CAKES.contains(state.getType())) {
            ...
            cake.setBites(1);
            player.world.updateBlock(place.getPlacedAgainstBlockLocation(), cake);
            return true;
        }
        if (state.getType() == StateTypes.CAKE) {
            if (state.getBites() == 0 && BlockTags.CANDLES.contains(place.getMaterial())) {
                player.world.updateBlock(place.getPlacedAgainstBlockLocation(), StateTypes.CANDLE_CAKE.createBlockState(CompensatedWorld.blockVersion));
                return true;
            }
            ...
            if (player.gamemode == GameMode.CREATIVE || (player.food < 20)) {
                if (state.getBites() != 6) {
                    ...
                    player.world.updateBlock(place.getPlacedAgainstBlockLocation(), state);
                } else {
                    player.world.updateBlock(place.getPlacedAgainstBlockLocation(), StateTypes.AIR.createBlockState(CompensatedWorld.blockVersion));
                }
                return true;
            }
