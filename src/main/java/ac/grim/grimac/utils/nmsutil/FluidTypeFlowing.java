...
public class FluidTypeFlowing {
    public static Vector getFlow(GrimPlayer player, int originalX, int originalY, int originalZ) {
        float fluidLevel = (float) Math.min(player.world.getFluidLevelAt(originalX, originalY, originalZ), 8 / 9D);
        ...
        for (BlockFace enumdirection : new BlockFace[]{BlockFace.NORTH, BlockFace.EAST, BlockFace.SOUTH, BlockFace.WEST}) {
            ...
            if (affectsFlow(player, originalX, originalY, originalZ, modifiedX, originalY, modifiedZ)) {
                float f = (float) Math.min(player.world.getFluidLevelAt(modifiedX, originalY, modifiedZ), 8 / 9D);
                float f1 = 0.0F;
                if (f == 0.0F) {
                    StateType mat = player.world.getBlockType(modifiedX, originalY, modifiedZ);
                    ...
                    if (Materials.isSolidBlockingBlacklist(mat, version)) {
                        if (affectsFlow(player, originalX, originalY, originalZ, modifiedX, originalY - 1, modifiedZ)) {
                            f = (float) Math.min(player.world.getFluidLevelAt(modifiedX, originalY - 1, modifiedZ), 8 / 9D);
                            if (f > 0.0F) {
        ...
        WrappedBlockState state = player.world.getBlock(originalX, originalY, originalZ);
    ...
    protected static boolean isSolidFace(GrimPlayer player, int originalX, int y, int originalZ, BlockFace direction) {
        ...
        WrappedBlockState data = player.world.getBlock(x, y, z);
    ...
    public static boolean isEmpty(GrimPlayer player, int x, int y, int z) {
        return player.world.getFluidLevelAt(x, y, z) == 0;
    }
    ...
    public static boolean isSame(GrimPlayer player, int x1, int y1, int z1, int x2, int y2, int z2) {
        return player.world.getWaterFluidLevelAt(x1, y1, z1) > 0 &&
                player.world.getWaterFluidLevelAt(x2, y2, z2) > 0 ||
                player.world.getLavaFluidLevelAt(x1, y1, z1) > 0 &&
                        player.world.getLavaFluidLevelAt(x2, y2, z2) > 0;
    }
}
